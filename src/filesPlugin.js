...
export function registerFilesPlugin() {
	const action = new FileAction({
		...
		iconSvgInline: () => `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none"
			stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
			<path d="M12 20h9"/>
			<path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/>
		</svg>`,

		// Only show for text/plain files the user may write (or at least read)
		enabled(nodes) {
			if (!nodes || nodes.length !== 1) return false
			const node = nodes[0]
			if (node.mime !== 'text/plain') return false
			return (node.permissions & Permission.READ) !== 0
		},

		// New @nextcloud/files v4 handler signature: ({ node, view, dir })
		async exec({ node }) {
			const fileId = node.fileid ?? node.fileId
			if (!fileId) return null
			const url = generateUrl(`/apps/simpletexteditor/edit/${fileId}`)
